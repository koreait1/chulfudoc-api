package org.backend.chulfudoc.file.services;

import com.querydsl.core.BooleanBuilder;
import jakarta.servlet.http.HttpServletRequest;
import lombok.RequiredArgsConstructor;
import org.backend.chulfudoc.file.constants.FileStatus;
import org.backend.chulfudoc.file.entities.FileInfo;
import org.backend.chulfudoc.file.entities.QFileInfo;
import org.backend.chulfudoc.file.exceptions.FileNotFoundException;
import org.backend.chulfudoc.file.repositories.FileInfoRepository;
import org.backend.chulfudoc.global.configs.FileProperties;
...
public class FileInfoService {
    ...
    public String getFileUrl(FileInfo item) {
        return utils.getUrl(String.format("%s/%s/%s", fileProperties.getUrl(), folder(item), item.getSeq() + Objects.requireNonNullElse(item.getExtension(), "")));
    }
